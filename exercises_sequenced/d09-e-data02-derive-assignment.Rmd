...
They produce the same output but the second version is piped which makes it clearer/easier to read.
...
It changes it from the mean of all the prices together to the mean of each combo of color and clarity.
...
df_q4 <-
  diamonds %>%
    #filter(x != 0, y != 0) %>% # issue with cov_diff being NA bc of /(0+0)
    mutate('depth_computed' = 100 * 2 * z / (x + y)) %>%
    mutate('diff' = depth - depth_computed) %>%
    filter(is.finite(diff)) %>% # ig i filtered out to much but this works
    summarize(
    ...
    mutate('cov_depth' = depth_sd / depth_mean) %>%
    mutate('cov_diff' = diff_sd / diff_mean) 
df_q4
```
...
- cov_diff is several orders of magnitude larger than cov_depth.
- Why is `cov_diff` so large?
- It looks like the diff tends to be very close to zero on average, so even though the standard deviation isn't massive, it makes the cov very high.
...
df_q5 <-
  diamonds %>%
    ...
    mutate('price_cov' = price_sd / price_mean)
