...
df_q1 %>%
  ...
  summarize(MeanVelocity = mean(Velocity), n = n()) %>%
  knitr::kable()
```
...
The higher the distinctness, the higher the mean velocity and the higher
the count.
...
VELOCITY_ERROR <- LIGHTSPEED_VACUUM - LIGHTSPEED_MICHELSON
...
is less than the true error. - Make a quantitative comparison between
Michelson’s uncertainty and his error. - His error bounds were +/- 51 km
/ s and the true error is -151.542 km / s. This means he was off by
almost exactly 100 km / s.
...
Similarities - They seem to have a similar spread in terms of mean and
in terms of individual data points. Both have a few measurements outside
of the error bounds.
...
``` r
df_q2 %>%
  ggplot(aes(factor(Distinctness), VelocityVacuum)) +
  geom_boxplot()  +
  geom_hline(
    yintercept = LIGHTSPEED_MICHELSON,
    linetype = "dotted"
...
``` r
ggplot(df_q2, aes(Date, VelocityVacuum, color = Distinctness)) +
  geom_point() +
  geom_hline(
    yintercept = LIGHTSPEED_MICHELSON,
    linetype = "dotted"
...
``` r
ggplot(df_q2, aes(Date, VelocityVacuum, color = Temp)) +
  geom_point() +
  geom_hline(
    yintercept = LIGHTSPEED_MICHELSON,
    linetype = "dotted"
...
- The distinctness plot shows that worse quality observations gave
  somewhat lower velocity estimates overall, though the distributions do
  have a lot of overlap. The date/distinctness plot shows that the
  measurements are varied in quiality throughout the experiment, with
  low distinctness tending to be lower in velocity. Medium and high
  distinctness are a lot more similar, though it is notable that none of
  the outliers are at a 3 in distinctness. The temperature plot doesn’t
  show any obvious relationship between temp and measured velocity.
  Taken together, these plots suggest that measurement quality may have
  contributed a bit to the variation in Michelson’s results though it is
  notable that the low quality push the velocity down, and Michelson’s
  estimate is higher than the true value. Temperature and do not appear
  to be a major factor. So overall, plots do not identify a clear/strong
  explanation for why Michelson’s estimate is 150 km/s from the true
  value.
